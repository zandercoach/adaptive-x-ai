...
**Where he talks:** `#crew` for everything normal — requests, answers, the weekly status (which also goes out as an email); `#crew-alerts` only when the coming week has no post scheduled at all. That is the entire escalation list, and it is enumerated rather than left to his judgement, so the alerts channel keeps its signal.

**The milestone:** on 2026-07-23 Morten went from borrowing Christian's credentials to holding his own — his own account, his own repo-scoped token, his own commit attribution. The first agent on this endeavor to become a team member *in the tooling,* not just in the description.

His full design — triggers, tools, boundaries, the paste-into-Abundly block, and the running log — is the versioned source of truth at [`agents/morten-market/MORTEN.md`](../agents/morten-market/MORTEN.md). This entry is the *who;* that file is the *how.*
