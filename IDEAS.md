...
19. **Give Morten an identity of his own in the repo** `[org]` `[eng]`
   The fine-grained token he got on 2026-07-21 belongs to Christian's account, so every commit and pull request Morten makes is authored by "zandercoach" — in the git history his work is indistinguishable from Christian's. For a journey whose whole subject is who did what, that is a gap worth closing: investigate a GitHub App with its own bot identity (or, as a stopgap, a committer identity like `Morten Market <morten@…>` on his commits), and check what that changes about permissions, the review gate, and the audit trail. *Why: the history is documentation here, not plumbing — and the first non-human contributor just made the question real. (From the 2026-07-21 session.)*
