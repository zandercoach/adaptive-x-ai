...
*Transcript of `C:\claude\VOICE.md`, as of 2026-09-19. Generated by `material/tooling/sync-voice.py`; changes belong in the source, not here.*
...
| Website (zander.coach) | `brand/positioning/VOICE.md` | Gender form, dose of self-irony, what does not carry over from LinkedIn |
...
| Slide decks | `material/decks/VOICE.md` | Who speaks on the slide, gender form; the slide carries fragments and the voice lives in the director's notes, so the dose is per slide instead of per text |
...
- **Address: du, not Sie.** And the distinction that is not mechanical: **du**
  ...
  meant ("euer Geschäftsmodell", "was ihr mit KI macht").
  **Lower case, except where it starts a sentence** — "du", "dich", "deine", not
  "Du", "Dich", "Deine". *Decided 2026-09-14 and moved here from the website
  supplement the same day: once every channel writes it the same way, it is not
  an exception but the rule. The counter-test applies — the reason would hold for
  a post just as well.*
...
- **Self-irony is required, not permitted.** Every surface that speaks as
  Christian carries at least one gesture of humility or a wink. Without it the
  text reads like a good template and not like him; that is the difference the
  website check made visible on 2026-08-25. **In moderation, and not capped at
  one** — *changed 2026-09-19, over the „Über mich" page: „exactly one" was the
  dose of that single check, written down as a principle. What makes a wink a
  shtick is the same move repeated, not the second one.* The irony is gentle
  and at his own expense, never snark, never at others'.
